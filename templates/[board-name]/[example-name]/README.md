---
sys: [OS Identifier]
sys_ver: [OS Version]
sys_var: [Variant Identifier]
provider: [Image Provider]
category: [Example Category]
last_update: [Last Update Date]

model: [Board Name]
profile: [Example Name]
---

<!-- `provider` is optional; replace the placeholder when known, or remove the field. -->
<!-- Use a canonical `category` value from "Example Categories" in CONTRIBUTING.md. -->

# RuyiSDK [Example Category] Example

- Install dependencies
- Install RuyiSDK
- Install the toolchain

## [Example Name]

- Describe the example and hardware preparation
- Create and activate a Ruyi virtual environment
- Compile the example with a Ruyi toolchain
- Run the example and verify the expected results
