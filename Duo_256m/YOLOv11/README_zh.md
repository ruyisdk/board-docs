...
## YOLOv11
### 示例描述和硬件环境准备
基于 TDL SDK 实现 YOLOv11 目标检测，读取图像并进行推理，验证 Ruyi 工具链在 Duo 256M（RISC-V）上的交叉编译能力。   
...
```
### 使用 ruyi 工具链编译示例代码
由于 Ultralytics 官方在设计 YOLOv11 时，延续了与 YOLOv8 相同的 Anchor-Free（无锚框）检测头架构，两者的模型输出张量（Tensor）维度和坐标解码逻辑高度一致。
因此，YOLOv11 可复用 sample_yolov8 示例程序进行推理验证。 
获取 TDL-SDK 示例源码   
```bash
...
### 运行示例并验证结果
将编译好的 sample_yolov8、yolo11n_cv181x_int8_sym.cvimodel、要推理的 jpg 图片，拷贝到板端然后执行二进制程序:  
```bash
scp sample_yolov8 test.jpg root@192.168.42.1:/root/
cd yolo11-obiect-dection
scp yolo11n_cv181x_int8_sym.cvimodel root@192.168.42.1:/root/
```
开发板上执行以下命令：  
```bash
...
objnum:2
boxes=[[749.426,40.0554,1150.87,711.339,0,0.870221],[151.084,198.936,1130.88,715.036,0,0.844418],]
